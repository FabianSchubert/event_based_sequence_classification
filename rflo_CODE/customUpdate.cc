...
// merged extra global parameter functions
// ------------------------------------------------------------------------
__device__ __constant__ unsigned int d_mergedCustomUpdateGroupStartID3[] = {0, 576, };
...
__device__ __constant__ unsigned int d_mergedCustomUpdateGroupStartID4[] = {0, 576, };
__device__ __constant__ unsigned int d_mergedCustomUpdateWUGroupStartID1[] = {672, 6240, 256320, };
extern "C" __global__ void customUpdatePlast(float t)
 {
    ...
    if(id < 672) {
        ...
        if(lid < group->size) {
            ...
            //lvariable += (5.00000000000000010e-04f) * lm / (sqrt(lv) +  (9.99999999999999955e-07f));
            lvariable += (5.00000000000000010e-04f) * m_hat / (sqrt(v_hat) +  (9.99999999999999955e-07f));
            ltime += 1.0f;
            group->m[lid] = lm;
            group->v[lid] = lv;
...
void updatePlast() {
     {
        const dim3 threads(96, 1);
        const dim3 grid(2728, 1);
        customUpdatePlast<<<grid, threads>>>(t);
        CHECK_CUDA_ERRORS(cudaPeekAtLastError());
    }
}
__device__ __constant__ unsigned int d_mergedCustomUpdateWUGroupStartID0[] = {0, 5568, 255648, };
